...
__global__ void cutlassSddmmKernel_target_16(
    ...
    int gemm_k_size,
    // target records the target index in each row
    int64_t* __restrict__ target = NULL,
    // target sp records the target index in each row in nnz
    int64_t* __restrict__ target_sp = NULL,
    ...
    cutlass::MatrixCoord mask_size = cutlass::MatrixCoord(0, 0))
{
    ...
    epilogue_sddmm.get_meta_data(accumulators, lane_idx, iterator_E, output_op_, threadblock_offset, target, target_sp);
    __syncthreads();
...
std::vector<torch::Tensor> sddmm_target_cuda(
    ...
    const float alpha_)
{
    ...
    auto target_sp = torch::empty_like(target);
    ...
    if (mask.has_value()){
        cudaFuncSetAttribute(cutlassSddmmKernel_target_16<typename Config::Element, typename Config::Mma, typename Config::SharedStorage, typename Config::Epilogue, typename Config::Prologue, true>, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);
        cudaFuncSetAttribute(cutlassSddmmKernel_target_16<typename Config::Element, typename Config::Mma, typename Config::SharedStorage, typename Config::Epilogue, typename Config::Prologue, true>, cudaFuncAttributePreferredSharedMemoryCarveout, 100);
        cutlass::MatrixCoord mask_shape{cutlass::MatrixCoord::Index(batch_size / (mask.value().numel()/n)), cutlass::MatrixCoord::Index(n)};
        cutlassSddmmKernel_target_16<
        ...
        typename Config::SharedStorage, typename Config::Epilogue, typename Config::Prologue, true><<<grid, block, smem_size>>>(
            ...
            target.data<int64_t>(), target_sp.data<int64_t>(),
            (typename Config::Element*)mask.value().data_ptr(), mask_shape
        );
    } else {
        cudaFuncSetAttribute(cutlassSddmmKernel_target_16<typename Config::Element, typename Config::Mma, typename Config::SharedStorage, typename Config::Epilogue, typename Config::Prologue, false>, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);
        cudaFuncSetAttribute(cutlassSddmmKernel_target_16<typename Config::Element, typename Config::Mma, typename Config::SharedStorage, typename Config::Epilogue, typename Config::Prologue, false>, cudaFuncAttributePreferredSharedMemoryCarveout, 100);
    ...
    return {output_matrix, metadata, target_sp};
}
    ...
    torch::optional<torch::Tensor> mask, const float alpha)
{
    ...
    if (k == 64){
        ...
        return sddmm_target_cuda<Config>(tensor_a, tensor_b, target, mask, alpha);
    } else if (k == 128){
