...
class SoftmaxBackwardArguments(ReduceApplyArguments):
    def __init__(
        ...
        grad: torch.Tensor, softmax: torch.Tensor, output_op, **kwargs) -> None:
        super().__init__(operation, problem_size, **kwargs)

        self.ptr_grad = self.tensor2ptr(grad)
        self.ptr_softmax = self.tensor2ptr(softmax)
    ...
    def get_arguments(epilogue_functor):
        ...
        class _SoftmaxArguments(ctypes.Structure):
            _fields_ = [
                ...
                ("ptr_softmax", ctypes.c_void_p),
                ("stride_input", stride_type),
                ("problem_size", MatrixCoord_),
                ("epilogue_args", _EpilogueOutputOpParams)
            ]
            def __init__(self, ptr_grad, ptr_softmax, problem_size, epilogue_args) -> None:
                self.ptr_grad = int(ptr_grad)
                self.ptr_softmax = int(ptr_softmax)
...
class SoftmaxBackwardOperation(ReduceApplyOperation):
    ...
    def procedural_name(self):
        return "softmax_backward_kernel"
...
class EmitSoftmaxBackwardUniversalInstance:
    def __init__(self, operation_suffix='') -> None:
        ...
        self.includes = [
            ...
            "softmax/threadblock/softmax_backward_reduction.h",
...
using ${operation_name}_reduction = 
    cutlass::softmax::threadblock::SoftmaxBackwardReduction<
        ${element},
        ${alignment},
        ${element_accumulator},
        OutputTileThreadMap,
