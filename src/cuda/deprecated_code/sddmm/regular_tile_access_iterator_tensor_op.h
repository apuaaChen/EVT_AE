...
class RegularTileAccessIteratorV3;
...
class RegularTileAccessIteratorV3<Shape_, Element_,
 ...
 public:
  ...
  struct Detail {
    ...
    /// Number of pointers
    /// In this case, only one pointer is required
    /// As the XOR offset is static for each thread
 ...
 public:
  ...
  CUTLASS_HOST_DEVICE
  RegularTileAccessIteratorV3(TensorRef ref,  ///< Pointer to start of tensor
    ...
    pointer_[0] = reinterpret_cast<AccessType *>(ref.data()) +
                    ref.offset(thread_offset_base) /
  ...
  AccessType *get() const {
    AccessType *access_ptr = pointer_[0];
    int stride_idx = iteration_strided_;
  ...
  CUTLASS_HOST_DEVICE
  RegularTileAccessIteratorV3 &operator++() {
  ...
  /// Advances to the next tile in memory.
  CUTLASS_HOST_DEVICE
  RegularTileAccessIteratorV3 operator++(int) {
    RegularTileAccessIteratorV3 prev(*this);
    this->operator++();
...
class RegularTileAccessIteratorV3<
 ...
 public:
  ...
  using UnderlyingIterator = RegularTileAccessIteratorV3<
