...
class TestSpMM(unittest.TestCase):
    def test_ntn_bf16(self):
        ...
        output_matrix_ref = torch.bmm(lhs_matrix, uncompressed).transpose(1, 2) * alpha
        output_matrix = spmmt_bf16_ntn(nonzeros, lhs_matrix, metadata, alpha)
    ...
    def test_ntt_bf16(self):
        ...
        lhs_matrix = torch.randn(size=(L, batch_size, feat_out), dtype=dtype, device="cuda")
        nonzeros, uncompressed, metadata = bdense2sparse_gold(dense_matrix, False)
        output_matrix_ref = torch.bmm(lhs_matrix, uncompressed) * alpha
        output_matrix = spmmt_bf16_ntt(nonzeros, lhs_matrix, metadata, alpha)
    ...
    def test_ntn_half(self):
        ...
        nonzeros, uncompressed, metadata = bdense2sparse_gold(dense_matrix, False)
        output_matrix_ref = torch.bmm(lhs_matrix, uncompressed).transpose(1, 2) * alpha
        output_matrix = spmmt_f16_ntn(nonzeros, lhs_matrix, metadata, alpha)
    ...
    def test_ntt_half(self):
        ...
        nonzeros, uncompressed, metadata = bdense2sparse_gold(dense_matrix, False)
        output_matrix_ref = torch.bmm(lhs_matrix, uncompressed) * alpha
        output_matrix = spmmt_f16_ntt(nonzeros, lhs_matrix, metadata, alpha)
    ...
    def test_nnn_bf16(self):
        ...
        output_matrix = spmmt_bf16_nnn(nonzeros, lhs_matrix, metadata, alpha)
    ...
    def test_nnt_bf16(self):
        ...
        lhs_matrix = torch.randn(size=(L, feat_out, batch_size), dtype=dtype, device="cuda")
        nonzeros, uncompressed, metadata = bdense2sparse_gold(dense_matrix, False)
        output_matrix_ref = torch.bmm(lhs_matrix.transpose(1, 2), uncompressed) * alpha
        output_matrix = spmmt_bf16_nnt(nonzeros, lhs_matrix, metadata, alpha)
    ...
    def test_nnn_half(self):
        ...
        nonzeros, uncompressed, metadata = bdense2sparse_gold(dense_matrix, False)
        output_matrix_ref = torch.bmm(lhs_matrix.transpose(1, 2), uncompressed).transpose(1, 2) * alpha
        output_matrix = spmmt_f16_nnn(nonzeros, lhs_matrix, metadata, alpha)
    ...
    def test_nnt_half(self):
        ...
        nonzeros, uncompressed, metadata = bdense2sparse_gold(dense_matrix, False)
        output_matrix_ref = torch.bmm(lhs_matrix.transpose(1, 2), uncompressed) * alpha
        output_matrix = spmmt_f16_nnt(nonzeros, lhs_matrix, metadata, alpha)
