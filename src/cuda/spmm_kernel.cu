...
using Mma_bf16_nt = typename cutlass::gemm::threadblock::DefaultSparseMma<
    cutlass::bfloat16_t, cutlass::layout::RowMajor, 128 / cutlass::sizeof_bits<cutlass::bfloat16_t>::value,
...
using Epilogue_bf16_nt = typename cutlass::epilogue::threadblock::DefaultEpilogueTensorOp<
    ThreadblockShape_bf16, typename Mma_bf16_nt::Operator, ThreadblockShape_bf16::kK / WarpShape_bf16::kK, EpilogueOp_bf16,
    EpilogueOp_bf16::kCount>::Epilogue;
...
union SharedStorage_bf16_nt {
    typename Mma_bf16_nt::SharedStorage main_loop;
    typename Epilogue_bf16_nt::SharedStorage epilogue;
};
...
__device__ void cutlassSpmmKernel_bf16_nt_(
    cutlass::gemm::GemmCoord problem_size,
    cutlass::gemm::GemmCoord grid_tiled_shape,
    typename Mma_bf16_nt::IteratorA::Params params_A,
    cutlass::bfloat16_t* __restrict__ ptr_A,
    typename Mma_bf16_nt::IteratorB::Params params_B,
    cutlass::bfloat16_t* __restrict__ ptr_B,
    typename Epilogue_bf16::OutputTileIterator::Params params_D,
    cutlass::bfloat16_t* __restrict__ ptr_D,
    typename Mma_bf16_nt::IteratorE::Params params_E,
    Mma_bf16_nt::ElementE* __restrict__ ptr_E,
    typename Epilogue_bf16::OutputOp::Params output_op_,
    int gemm_k_size)
{
    extern __shared__ int SharedStorageBase[];

    SharedStorage_bf16_nt& shared_storage = *reinterpret_cast<SharedStorage_bf16_nt *>(SharedStorageBase);
    ...
    // Compute initial location in logical coordinates
    cutlass::MatrixCoord tb_offset_A{
        threadblock_tile_offset.m() * Mma_bf16_nt::Shape::kM,
        threadblock_tile_offset.k() * gemm_k_size / Mma_bf16_nt::kSparse
    };

    cutlass::MatrixCoord tb_offset_B{
        threadblock_tile_offset.k() * gemm_k_size,
        threadblock_tile_offset.n() * Mma_bf16_nt::Shape::kN
    };
    ...
    int gemm_k_iterations = (problem_size_k - tb_offset_B.row() + Mma_bf16_nt::Shape::kK - 1) / Mma_bf16_nt::Shape::kK;

    // Compute position within threadblock
    int thread_idx = threadIdx.x;

    // Construct iterators to A, B, and E operands
    typename Mma_bf16_nt::IteratorA iterator_A(
        params_A,
        //ref_A.data(),
        ptr_A,
        {problem_size.m(), problem_size_k / Mma_bf16_nt::kSparse},
        thread_idx,
        tb_offset_A
    );

    typename Mma_bf16_nt::IteratorB iterator_B(
        params_B,
        //ref_B.data(),
        ptr_B,
        {problem_size_k, problem_size.n()},
    ...
    typename Mma_bf16_nt::IteratorE iterator_E(
        params_E,
        // ref_E.data(),
        ptr_E,
        {problem_size.m(),
        problem_size_k / Mma_bf16_nt::kSparse / Mma_bf16_nt::kElementsPerElementE},
        thread_idx,
        tb_offset_E
    );
    ...
    // Construct thread-scoped matrix multiply
    Mma_bf16_nt mma(shared_storage.main_loop, thread_idx, warp_idx, lane_idx);

    typename Mma_bf16_nt::FragmentC accumulators;
    ...
    Epilogue_bf16_nt::OutputOp output_op(output_op_);
    ...
    typename Epilogue_bf16_nt::OutputTileIterator iterator_D(
        params_D,
        ptr_D,
        problem_size.mn(),
        thread_idx,
...
__global__ void cutlassSpmmKernel_bf16_nt(
    cutlass::gemm::GemmCoord problem_size,
    cutlass::gemm::GemmCoord grid_tiled_shape,
    typename Mma_bf16_nt::IteratorA::Params params_A,
    cutlass::bfloat16_t* __restrict__ ptr_A,
    ...
    typename Mma_bf16_nt::IteratorE::Params params_E,
    Mma_bf16_nt::ElementE* __restrict__ ptr_E,
    typename Epilogue_bf16_nt::OutputOp::Params output_op_,
    int gemm_k_size)
{
    cutlassSpmmKernel_bf16_nt_(
        problem_size, grid_tiled_shape,
        params_A, ptr_A, params_B, ptr_B,
        params_D, ptr_D, params_E, ptr_E,
        output_op_, gemm_k_size
    ...
    torch::Tensor tensor_e_reordered)
{
    ...
    auto layout_e = Mma_bf16::LayoutE::packed(cutlass::make_Coord(problem_size.m(), problem_size.k()/Mma_bf16::kSparse / Mma_bf16::kElementsPerElementE));
    ...
    torch::Tensor tensor_e_reordered)
{
    ...
    auto layout_e = Mma_bf16_nt::LayoutE::packed(cutlass::make_Coord(problem_size.m(), problem_size.k()/Mma_bf16_nt::kSparse / Mma_bf16_nt::kElementsPerElementE));
    auto layout_d = cutlass::layout::RowMajor::packed(problem_size.mn());
    ...
    dim3 grid = threadblock_swizzle.get_grid_shape(grid_tiled_shape);
    dim3 block(Mma_bf16_nt::WarpCount::kCount * 32, 1, 1);

    int smem_size = int(sizeof(SharedStorage_bf16));

    cudaFuncSetAttribute(cutlassSpmmKernel_bf16_nt, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);
    cudaFuncSetAttribute(cutlassSpmmKernel_bf16_nt, cudaFuncAttributePreferredSharedMemoryCarveout, 100);

    int gemm_k_size = ((problem_size.k() + Mma_bf16_nt::Shape::kK - 1) / Mma_bf16_nt::Shape::kK) * Mma_bf16_nt::Shape::kK;

    cutlassSpmmKernel_bf16_nt<<<grid, block, smem_size>>>(
        problem_size, grid_tiled_shape, 
        layout_a, (cutlass::bfloat16_t*)tensor_a.data_ptr(),
        layout_b, (cutlass::bfloat16_t*)tensor_b.data_ptr(),
        layout_d, (cutlass::bfloat16_t*)output_matrix.data_ptr(),
        layout_e, (Mma_bf16_nt::ElementE*)tensor_e_reordered.data_ptr(),
        {alpha, beta}, gemm_k_size);
