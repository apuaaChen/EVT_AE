...
from bert_modeling_slapo import xFlashAttention, LSBertEmbeddings, LSBertLayer
...
def pass_using_lightseq2_encoder_layer(sch):
    for subsch in sch.child:
        if isinstance(sch[subsch].mod, modeling.BertLayer):
...
def benchmarking_lightseq2():
    from lightseq_build_bert import LSBert
...
def benchmarking():
    ...
    if args.method == "east":
        ...
        raise NotImplementedError("Lightseq suffers from OOM in this script")
        import slapo
        sch = slapo.create_schedule(model, group=None)
        pass_using_lightseq2_embedding(sch)
...
if __name__ == '__main__':
    if args.unittest: 
        ...
        if args.method == "lightseq":
            benchmarking_lightseq2()
        else:
            benchmarking()