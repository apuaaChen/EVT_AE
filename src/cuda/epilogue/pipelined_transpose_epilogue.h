...
public:
    ...
    )
    {
        ...
        shared_store_ptr = reinterpret_cast<ShmemStoreAccessType *>(shared_storage) + shared_store_offset / kShmemStoreElementsPerAccess;
        ...
        int global_offset = (sub_warp_idx + group_idx * WarpTile_Shape::kN) * problem_size.m() + sub_lane_idx * kShmemLoadElementsPerAccess;

        shared_load_ptr = reinterpret_cast<float4 *>(shared_storage) + shared_load_offset / kShmemLoadElementsPerAccess;
        global_ptr = reinterpret_cast<float4 *>(ptr_D + threadblock_offset.column() * problem_size.m() + threadblock_offset.row()) + global_offset / kShmemLoadElementsPerAccess;
    }
