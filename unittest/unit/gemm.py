...
class GemmTestSm80(unittest.TestCase):
    ...
    def test_weight_gradient_mm_default_11(self):
        ## Define model to trace
        class WeightGradient(torch.nn.Module):
            def __init__(self) -> None:
                super().__init__()
        ...
        ## run the compiler pass
        symbolic_traced : torch.fx.GraphModule = symbolic_trace(module)
        gradient_input = torch.randn((32, 1024), dtype=torch.float16, device="cuda")
        input = torch.randn((32, 1024), dtype=torch.float16, device="cuda")
