 ...
 public:
  ...
  void set_predicates(int thread_id, TensorCoord const &threadblock_offset) {
    ...
    if (kAdvanceRank) {
      ...
      // extent: the whole matrix to access
      // for sparse iterator A, the extent is [m/2, k]
      // the threadblock_offset is [m * TbM/2, 0]
      // the Shape is [TbM/2, TbK]
  ...
  CUTLASS_HOST_DEVICE
  PredicatedTileAccessIteratorPredicatesTrans &operator++() {
...
/// PredicatedTileAccessIteratorTrans
///
template <typename Shape, typename Element, typename Layout, int AdvanceRank,
          typename ThreadMap, typename AccessType>
class PredicatedTileAccessIteratorTrans;

////////////////////////////////////////////////////////////////////////////////

/// Specialization of PredicatedTileAccessIteratorTrans for pitch-linear data.
///
template <typename Shape_, typename Element_, int AdvanceRank,
          typename ThreadMap_, typename AccessType_>
class PredicatedTileAccessIteratorTrans<Shape_, Element_, layout::PitchLinear,
 ...
 public:
  ...
  using UnderlyingPredicates = PredicatedTileAccessIteratorPredicatesTrans<
 ...
 public:
  ...
  /// Construct a PredicatedTileAccessIteratorTrans with zero threadblock offset
  CUTLASS_HOST_DEVICE
  PredicatedTileAccessIteratorTrans(
      /// Precomputed parameters object
      Params const &params,
      ...
      ///< ID of each participating thread
      int thread_id)
      : PredicatedTileAccessIteratorTrans(params, pointer, extent, thread_id,
                                     make_Coord(0, 0)) {}
  ...
  /// Increment and return an instance to self.
  CUTLASS_HOST_DEVICE
  PredicatedTileAccessIteratorTrans &operator++() {
  ...
  /// Increment and return an instance to self.
  CUTLASS_HOST_DEVICE
  PredicatedTileAccessIteratorTrans operator++(int) {
    PredicatedTileAccessIteratorTrans self(*this);
    operator++();
    return self;
  }
...
/// Specialization of PredicatedTileAccessIteratorTrans for row-major data.
///
...
template <typename Shape_, typename Element_, int AdvanceRank,
          typename ThreadMap_, typename AccessType_>
class PredicatedTileAccessIteratorTrans<Shape_, Element_, layout::RowMajor,
                                   AdvanceRank, ThreadMap_, AccessType_> {
 public:
  static_assert(
      AdvanceRank == 0 || AdvanceRank == 1,
  ...
  using UnderlyingIterator = PredicatedTileAccessIteratorTrans<
  ...
  class Params {
   private:
    friend PredicatedTileAccessIteratorTrans;
